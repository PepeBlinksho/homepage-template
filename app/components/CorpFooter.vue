  ...
  <footer class="bg-slate-950 text-slate-400">
    <div class="max-w-6xl mx-auto px-4 py-12">
      <div class="grid md:grid-cols-3 gap-10 mb-10">
        ...
        <!-- ナビゲーション -->
        <div>
          <h4 class="text-sm font-semibold text-white uppercase tracking-wider mb-4">メニュー</h4>
          <ul class="space-y-2">
        ...
        <!-- 所在地 -->
        <div>
          <h4 class="text-sm font-semibold text-white uppercase tracking-wider mb-4">所在地・連絡先</h4>
          <address class="not-italic text-sm space-y-1 text-slate-500">
            <p>〒{{ cfg.address.zip }}</p>
            <p>{{ buildCorpAddress(cfg.address) }}</p>
            <p class="mt-2">
              TEL:
              <a :href="`tel:${cfg.tel}`" class="hover:text-white transition-colors">{{ cfg.tel }}</a>
            </p>
            <p>
              <a :href="`mailto:${cfg.email}`" class="hover:text-white transition-colors">{{ cfg.email }}</a>
            </p>
            <p v-if="cfg.license" class="text-xs text-slate-600 mt-2">{{ cfg.license }}</p>
          </address>
        </div>
      </div>
      ...
      <div class="border-t border-slate-800 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-slate-600">
        ...
        <NuxtLink :to="`${prefix}/privacy`" class="hover:text-white transition-colors">
          プライバシーポリシー
        </NuxtLink>
      </div>
    </div>
