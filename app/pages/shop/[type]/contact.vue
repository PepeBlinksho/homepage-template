...
const shopMeta = computed(() => ({
  shopName: siteConfig.value.name,
  shopTel: siteConfig.value.tel,
  shopAddress: buildFullAddress(siteConfig.value.address),
}))
  ...
  <div class="pt-16 min-h-screen bg-white">
    <div class="relative bg-stone-900 py-24 overflow-hidden">
      ...
      <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6">
        <p class="text-xs tracking-[0.5em] uppercase text-amber-400 mb-3">Contact</p>
        <h1 class="font-serif text-5xl md:text-6xl font-semibold text-white">お問い合わせ</h1>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-20">
      <div v-if="submitted" class="max-w-lg mx-auto text-center py-16">
        <div class="w-20 h-20 rounded-full bg-amber-50 flex items-center justify-center mx-auto mb-8">
          <UIcon name="i-heroicons-check-circle" class="w-10 h-10 text-amber-500" />
        </div>
        <h2 class="font-serif text-3xl font-semibold text-stone-900 mb-4">送信が完了しました</h2>
        <p class="text-stone-500 mb-10 leading-relaxed">
          お問い合わせありがとうございます。<br>内容を確認のうえ、ご連絡いたします。
        </p>
        <UButton :to="prefix || '/'" color="primary" size="lg">トップページへ戻る</UButton>
      </div>
      ...
      <div v-else ref="el" class="grid md:grid-cols-5 gap-12 lg:gap-20">
        ...
        >
          <h2 class="font-serif text-2xl font-semibold text-stone-900 mb-6">お気軽にどうぞ</h2>
          <p class="text-stone-500 leading-relaxed mb-10 text-sm">
            ご質問・ご予約・その他何でもお気軽にお問い合わせください。通常2〜3営業日以内にご返信いたします。
          </p>
          <div class="space-y-6">
            <div class="flex items-start gap-4">
              <div class="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center shrink-0 mt-0.5">
                <UIcon name="i-heroicons-map-pin" class="w-4 h-4 text-amber-600" />
              </div>
              <div>
                <p class="text-xs text-stone-400 uppercase tracking-wider mb-1">住所</p>
                <p class="text-sm text-stone-700">{{ buildFullAddress(siteConfig.address) }}</p>
              </div>
            </div>
            <div class="flex items-start gap-4">
              <div class="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center shrink-0 mt-0.5">
                <UIcon name="i-heroicons-phone" class="w-4 h-4 text-amber-600" />
              </div>
              <div>
                <p class="text-xs text-stone-400 uppercase tracking-wider mb-1">電話</p>
                <a :href="`tel:${siteConfig.tel}`" class="text-sm text-stone-700 hover:text-amber-600 transition-colors">{{ siteConfig.tel }}</a>
              </div>
            </div>
            <div class="flex items-start gap-4">
              <div class="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center shrink-0 mt-0.5">
                <UIcon name="i-heroicons-envelope" class="w-4 h-4 text-amber-600" />
              </div>
              <div>
                <p class="text-xs text-stone-400 uppercase tracking-wider mb-1">メール</p>
                <a :href="`mailto:${siteConfig.email}`" class="text-sm text-stone-700 hover:text-amber-600 transition-colors">{{ siteConfig.email }}</a>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <UForm :schema="schema" :state="state" class="space-y-6" @submit="onSubmit">
            <UFormField label="お名前" name="name" required>
              <UInput v-model="state.name" placeholder="山田 太郎" class="w-full" size="lg" />
            </UFormField>
            ...
            <UFormField label="お問い合わせ内容" name="message" required>
              <UTextarea v-model="state.message" placeholder="お問い合わせ内容をご記入ください" :rows="6" class="w-full" size="lg" />
            </UFormField>
