...
useSeoMeta({
  ...
  description: `${siteConfig.name}からのお知らせ一覧です。`,
  ogTitle: `お知らせ | ${siteConfig.name}`,
  ogDescription: `${siteConfig.name}からのお知らせ一覧です。`,
    ...
    >
      ...
      <ul v-else class="space-y-0">
        ...
        >
          ...
          >
            <time
              class="text-sm text-stone-400 w-28 shrink-0 pt-0.5"
              :datetime="item.date"
            >
            ...
            <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 flex-1 min-w-0">
              ...
              />
              <span class="text-stone-800 font-medium group-hover:text-amber-700 transition-colors line-clamp-1">
                {{ item.title }}
              </span>
            </div>
            <UIcon
              name="i-heroicons-arrow-right"
              class="w-4 h-4 text-stone-300 group-hover:text-amber-500 transition-colors shrink-0 mt-0.5 sm:mt-0"
            />
          </NuxtLink>
        </li>
      </ul>
