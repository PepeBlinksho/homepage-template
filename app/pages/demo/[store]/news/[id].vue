...
useSeoMeta({
  title: `${articleTitle} | ${siteConfig.value.name}`,
  description: articleDescription,
  ogTitle: `${articleTitle} | ${siteConfig.value.name}`,
  ...
  <div class="pt-16 min-h-screen bg-white">
    <div class="relative bg-stone-900 py-24 overflow-hidden">
      ...
      <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <p class="font-serif text-5xl md:text-6xl font-semibold text-white">お知らせ</p>
        <div class="w-12 h-0.5 bg-amber-500 mt-4" />
      </div>
    </div>

    <div v-if="article" ref="el" class="max-w-2xl mx-auto px-4 sm:px-6 py-20">
      <div class="transition-all duration-700" :class="revealed ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'">
        <div class="flex items-center gap-3 mb-6">
          <time class="text-sm text-stone-400" :datetime="article.date">{{ formatDate(article.date) }}</time>
          <UBadge :label="article.category" color="primary" variant="subtle" />
        </div>
        <h1 class="font-serif text-2xl md:text-3xl font-semibold text-stone-900 mb-10 leading-snug">{{ article.title }}</h1>
        <div class="w-full h-px bg-gradient-to-r from-amber-400 to-transparent mb-10" />
        <div class="text-stone-700 leading-loose whitespace-pre-wrap text-base">{{ article.body }}</div>
        <div class="mt-14 pt-8 border-t border-stone-100">
          <UButton :to="`${prefix}/news`" variant="ghost" color="neutral" icon="i-heroicons-arrow-left" class="text-stone-500 hover:text-stone-900">
            一覧に戻る
          </UButton>
        </div>
      </div>
